...
for year, group in itertools.groupby(pubs_sorted, key=lambda p: int(p.get("year", 0))):
    html_parts.append(f'<section class="year-chunk"><div class="year-label" aria-hidden="true">{year}</div>')
    for pub in group:
...
html_parts.append('<div class="show-more-wrap"><button id="show-more-pubs" class="btn btn-primary">Show More Publications</button></div>')
...
## Technical Talks & Presentations {.section-divider}
...
"""
    ...
    talk_index += 1
rows.append("</div>")
rows.append('<div class="show-more-wrap"><button id="show-more-talks" class="btn btn-primary">Show More Talks</button></div>')
